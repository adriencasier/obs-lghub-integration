...
bool logi::applets::obs_plugin::connect()
{
	...
	m_compressor_ready_cv.wait_until(lk,
...
ws_client::connection_ptr logi::applets::obs_plugin::_create_connection(void)
{
	...
	auto connection =
		m_websocket.get_connection("ws://127.0.0.1:" + std::to_string(m_current_port), error_code);
...
void logi::applets::obs_plugin::websocket_message_handler(
	websocketpp::connection_hdl connection_handle, ws_client::message_ptr response)
{
	...
	if (!payload.empty()) {
		try {
			...
			if (!message["result"].is_null() &&
				(message["result"]["code"].get<std::string>() != "SUCCESS")) {
			...
			if ((message["verb"].get<std::string>() == "BROADCAST") &&
				...
				if (!instance_info_payload.is_null()) {
					std::string new_integration_guid =
						instance_info_payload["integrationGuid"].get<std::string>();
					std::string new_integration_instance =
						instance_info_payload["instanceGuid"].get<std::string>();
...
void logi::applets::obs_plugin::websocket_close_handler(
...
bool logi::applets::obs_plugin::register_integration()
{
	...
	nlohmann::json register_app = {{"path", "/api/v1/integration/register"},
								   {"verb", "SET"},
								   {"payload",
...
bool logi::applets::obs_plugin::initialize_actions()
{
	...
	nlohmann::json initalize_actions = {
		...
		{"verb", "SET"},
		{"payload", {{"integrationIdentifier", s_integration_identifier}, {"sdkType", "ACTION"}}}};
...
void logi::applets::obs_plugin::uninitialize_actions()
{
	...
	nlohmann::json uninitalize_actions = {{"path", "/api/v1/integration/deactivate"},
										  {"verb", "SET"},
										  {"payload",
										   {{"integrationIdentifier", s_integration_identifier},
														  ...
														  action_parameters parameters)
{
	nlohmann::json action = {
		{"actionId", action_id}, {"name", action_name}, {"parameters", parameters}};
...
bool logi::applets::obs_plugin::register_actions_broadcast()
{
	...
	nlohmann::json message = {{"path", "/api/v1/integration/sdk/action/invoke"},
							  {"verb", "SUBSCRIBE"}};
...
void logi::applets::obs_plugin::register_regular_actions()
{
	...
	actions.push_back(
		register_action(actions::s_desktop_mute_toggle, "APPLET_OBS_DESKTOP_MUTE_TOGGLE"));
	...
	nlohmann::json register_actions = {
		...
		{"payload",
		 {{"actions", actions},
		  {"instance",
		   {{"integrationGuid", m_integration_guid}, {"instanceGuid", m_integration_instance}}}}}};
...
void logi::applets::obs_plugin::register_parameter_actions()
{
	...
	std::vector<nlohmann::json> actions;
	actions.push_back(register_action(actions::s_collection_activate,
									  "APPLET_OBS_COLLECTION_ACTIVATE", collections));
	actions.push_back(
		register_action(actions::s_scenes_activate, "APPLET_OBS_SCENE_ACTIVATE", scenes));
	actions.push_back(
		register_action(actions::s_source_activate, "APPLET_OBS_SOURCE_ACTIVATE", sources));
	actions.push_back(
		register_action(actions::s_source_deactivate, "APPLET_OBS_SOURCE_DEACTIVATE", sources));
	actions.push_back(
		register_action(actions::s_source_toggle, "APPLET_OBS_SOURCE_TOGGLE", sources));
	...
	actions.push_back(
		register_action(actions::s_mixer_mute_toggle, "APPLET_OBS_MIXER_MUTE_TOGGLE", mixers));
...
void logi::applets::obs_plugin::helper_source_activate(std::string scene_name,
													   std::string source_name, bool new_state,
													   bool is_toggle)
{
	obs_frontend_source_list scenes = {};
	obs_frontend_get_scenes(&scenes);
	for (size_t i = 0; i < scenes.sources.num; i++) {
		...
		if (str_name == scene_name) {
			...
			auto sourceEnumProc = [](obs_scene_t *scene, obs_sceneitem_t *currentItem,
...
void logi::applets::obs_plugin::helper_mixer_mute(std::string scene_name, std::string mixer_name,
												  bool new_state, bool is_toggle)
{
	obs_frontend_source_list scenes = {};
	obs_frontend_get_scenes(&scenes);
	for (size_t i = 0; i < scenes.sources.num; i++) {
...
action_parameters logi::applets::obs_plugin::helper_get_available_collections()
{
	...
	nlohmann::json collections_parameter = {{"parameterId", actions::parameters::s_collection_name},
											{"assignmentMessage",
											 actions::messages::s_collection_assignment},
...
action_parameters logi::applets::obs_plugin::helper_get_available_scenes()
{
	...
	nlohmann::json scene_parameter = {{"parameterId", actions::parameters::s_scene_name},
...
action_parameters logi::applets::obs_plugin::helper_get_available_sources()
{
	...
	for (auto scene : collection) {
		...
		nlohmann::json source_parameter = {{"parameterId", actions::parameters::s_source_name},
										   {"assignmentMessage",
											actions::messages::s_source_assignment},
										   {"errorMessage", actions::messages::s_source_error},
										   {"parameterType", "LIST"},
										   {"listSelection", list_selection_source}};

		nlohmann::json selection = {{"displayName", scene.first},
									{"value", scene.first},
...
action_parameters logi::applets::obs_plugin::helper_get_available_mixers()
{
	...
	for (auto scene : collection) {
		...
		nlohmann::json mixer_parameter = {{"parameterId", actions::parameters::s_mixer_name},
										  {"assignmentMessage",
										   actions::messages::s_mixer_assignment},
...
void logi::applets::obs_plugin::helper_populate_collections()
{
	...
	for (char *collection_name = *collection_names; collection_name;
	...
	for (size_t i = 0; i < scenes.sources.num; i++) {
		...
		for (auto scene_element : obs_scenes->sources) {
			m_obs_collections[std::string(current_collection)][std::string(name)].sources.push_back(
				scene_element);
		}

		for (auto scene_element : obs_scenes->mixers) {
			m_obs_collections[std::string(current_collection)][std::string(name)].mixers.push_back(
				scene_element);
		}
...
void logi::applets::obs_plugin::loop_function()
{
	...
	while (true) {
		...
		nlohmann::json new_status = {{"path", "/api/v1/integration/obs/status"}, {"verb", "SET"}};
		...
		if (nullptr != obs_output) {
			...
			fps = static_cast<double_t>(streamed_frames) -
				  static_cast<double_t>(m_total_streamed_frames);
