...
struct new_state_info {
	...
	bool is_toggle;
	new_state_info()
		: name(std::string()), new_state(false), is_toggle(false)
	{
	}
};
...
nlohmann::json
register_action(std::string action_id, std::string action_name,
...
void helper_source_activate(std::string scene_name, std::string source_name,
			    bool new_state, bool is_toggle);
void helper_mixer_mute(std::string scene_name, std::string mixer_name,
		       bool new_state, bool is_toggle);
...
websocketpp::frame::opcode::value m_subprotocol_opcode =
